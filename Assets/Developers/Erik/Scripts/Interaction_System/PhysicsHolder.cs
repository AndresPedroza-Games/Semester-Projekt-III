...
public class PhysicsHolder : MonoBehaviour {
	...
	public void Hold(Holdable newHoldable, IHoldTargetResolver targetResolver, HoldPhysicsProfile profile, Vector3 hitPoint) {
		...
		Vector3 pullGrabOffset;
		if (profile.useHitPoint)
			pullGrabOffset = holdPoint.InverseTransformVector(hitPoint - holdPoint.position);
		else
			pullGrabOffset = _holdable.Rigidbody.position - holdPoint.position;
		pullGrabOffset.y = 0f;
